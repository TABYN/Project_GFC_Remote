...
from scolar.forms import EnseignantDetailForm, AbsenceEtudiantReportSelectionForm, SeanceEtudiantSelectionForm, \
    ...
    Fiche_regularisation_provision_CreateForm, Fiche_regularisation_provision_UpdateForm, Fiche_regularisation_provision_DetailForm
...
class Fiche_regularisation_provision_ListView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        #if self.request.user.is_staff_only():
        context['btn_list'] = {
            'Ajouter nouvelle Fiche de régularisation de la provision': reverse('Fiches_regularisation_provision_create'),
                  
            }
        return context

@login_required
def fiche_regularisation_provision_create_view(request):
    if request.method == 'POST':
        # create a form instance and populate it with data from the request:
        form = Fiche_regularisation_provision_CreateForm(request, request.POST)
        # check whether it's valid:
        if form.is_valid():
            try:
                # process the data in form.cleaned_data as required
                ...
                if settings.DEBUG:
                    ...
                    messages.error(request, "ERREUR: lors de la création de la fiche de regularisation de la provision. Veuillez le signaler.")
                    return render(request, 'scolar/create.html', {'form': form })

            return HttpResponseRedirect(reverse('Fiches_regularisation_provision_List'))
    
    # if a GET (or any other method) we'll create a blank form
    else:
        form = Fiche_regularisation_provision_CreateForm(request)
        messages.info(request, "Utilisez ce formulaire pour ajouter une nouvelle fiche de regularisation de la provision")
...
@login_required
def fiche_regularisation_provision_update_view(request, engagement_pk):
    engagement_=get_object_or_404(Engagement, id=engagement_pk)
    if request.user.is_budget():
        pass       
    else :
    ...
    if request.method == 'POST':
        # create a form instance and populate it with data from the request:
        form = Fiche_regularisation_provision_UpdateForm(engagement_pk, request, request.POST)
        # check whether it's valid:
        if form.is_valid():
            try:
                # process the data in form.cleaned_data as required
                data=form.cleaned_data
                engagement_.id=data['id']      
                engagement_.annee_budg=data['annee_budg']
                engagement_.type=data['type']
                engagement_.date=data['date']
                engagement_.num=data['num']
    ...
    # if a GET (or any other method) we'll create a blank form
        ...
        form = Fiche_regularisation_provision_UpdateForm(engagement_pk, request)
        messages.info(request, "Utilisez ce formulaire pour modifier l'engagement")
...
class Fiche_regularisation_provisionDeleteView(LoginRequiredMixin, SuccessMessageMixin, UserPassesTestMixin, DeleteView):
    model = Engagement
    template_name = 'scolar/delete.html'
    success_message = "La Fiche de regularisation de la provision àété bien supprime."
...
class Fiche_regularisation_provision_DetailView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(Fiche_regularisation_provision_DetailView, self).get_context_data(**kwargs)
        titre='Engagement Fiche de regularisation de la provision '#+ self.kwargs.get("pk")
        context['titre'] = titre

        engagement_=get_object_or_404(Engagement, id=self.kwargs.get("pk"))
        
        context['engagement_form'] = Fiche_regularisation_provision_DetailForm(engagement_pk=engagement_.id)
...
class MandatListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        filter_ = Mandat_1_Filter(self.request.GET, queryset=Credit_S2.objects.filter(exercice__exe_encours=True).filter(article__posteriori=True))
        ...
        context['filter'] = filter_
        context['table'] = table
        context['titre'] = 'Liste des Mandats a posteriori '
...
class Mandat_PrioriListView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['filter'] = filter_
        context['table'] = table
        context['titre'] = 'Liste des Mandats a priori'
        #if self.request.user.is_staff_only():
        context['btn_list'] = {
    ...
    if request.method == 'POST':
        ...
        if semestre == 's1' or semestre == 'S1':
            ...
            if transferts:
                ...
                return render(request, 'scolar/transfert_post_depense.html', {'crd':crd, 'credit_s2_': credit_s2_ ,'transferts': transferts, 'article_source': article_source, 'destination':destination, 'annee_bdg':annee_bdg, 'somme_montant_transfert_post':somme_montant_transfert_post,'semestre':semestre, 'somme_montant_transfert_post':somme_montant_transfert_post})#  'mois_debut':mois_debut, 'mois_fin':mois_fin
            else:
                messages.error(request, "Il y a pas de transfert dans le 1er Semestre pour cette Article ")
        elif semestre == 's2' or semestre == 'S2':
...
class Transfert_moins_posteriori_PDFView(PDFTemplateView):
    ...
    titre = 'Les depense d un article a posteriori'
    cmd_options = {
        'orientation': 'Landscape',
        'page-size': 'A3',
    }
    ...
    def get_context_data(self,  **kwargs):
        semestr = kwargs.get('semestre') # Accs  la variable de semestre
        print('semestr:', semestr)
        ...
        if semestr == 's1' or semestr == 'S1':
            all_transferts = all_transferts_s1
                        
        elif semestr == 's2' or semestr == 'S2':
            all_transferts = all_transferts_s2  
...
def Transfert_economie(request, crd): # c est le meme code que (def Transfert_depense(request, crd):)sof qu'il faut remplacer article_source par article destination
 ...
 #   print(art_destination_id) 
    ...
    date_debut_s1 = datetime.date(int(annee_bdg.annee_univ), 1, 1)         
    date_fin_s1 = datetime.date(int(annee_bdg.annee_univ), 6, 30)  
    all_transferts_s1=  Transfert.objects.filter(date_transfert__range=[date_debut_s1, date_fin_s1]).filter(annee_budgi=annee_bdg).filter(article_destination_id=crd)#.exclude(credit_s2__isnull=True)
    #recuperer les transferts du 2eme semestre
    date_debut_s2 = datetime.date(int(annee_bdg.annee_univ), 7, 1)  
    date_fin_s2 = datetime.date(int(annee_bdg.annee_univ), 12, 31)  
    ...
    if request.method == 'POST':
        ...
        if semestre == 's1' or semestre == 'S1':
            ...
            if transferts:
                ...
                print(somme_montant_transfert_post)
                return render(request, 'scolar/transfert_post_economie.html', {'crd':crd, 'credit_s2_': credit_s2_ ,'transferts': transferts, 'article_destination': article_destination, 'source':source, 'annee_bdg':annee_bdg, 'somme_montant_transfert_post':somme_montant_transfert_post,'semestre':semestre, 'somme_montant_transfert_post':somme_montant_transfert_post})#  'mois_debut':mois_debut, 'mois_fin':mois_fin
            else:
                messages.error(request, "Il y a pas de transfert dans le 1er Semestre pour cette Article ")
        elif semestre == 's2' or semestre == 'S2':
            transferts = all_transferts_s2 
...
class Transfert_plus_posteriori_PDFView(PDFTemplateView):
    template_name= 'scolar/transfert_plus_posteriori.html'
    titre = 'Les Economies d un article a posteriori'
    cmd_options = {
        'orientation': 'Landscape',
        'page-size': 'A3',
    }
    ...
    def get_context_data(self,  **kwargs):
        ...
        date_debut_s1 = datetime.date(int(annee_bdg.annee_univ), 1, 1)         
        date_fin_s1 = datetime.date(int(annee_bdg.annee_univ), 6, 30)  
        all_transferts_s1=  Transfert.objects.filter(date_transfert__range=[date_debut_s1, date_fin_s1]).filter(annee_budgi=annee_bdg).filter(article_destination=self.kwargs.get('credit_s2__pk'))#.exclude(credit_s2__isnull=True)
        print('all_transferts_s1')
        print(all_transferts_s1)

        date_debut_s2 = datetime.date(int(annee_bdg.annee_univ), 7, 1)  
        date_fin_s2 = datetime.date(int(annee_bdg.annee_univ), 12, 31)  
        all_transferts_s2=  Transfert.objects.filter(date_transfert__range=[date_debut_s2, date_fin_s2]).filter(annee_budgi=annee_bdg).filter(article_destination=self.kwargs.get('credit_s2__pk'))#.exclude(credit_s2__isnull=True)
        print('all_transferts_s2')
        print(all_transferts_s2)
