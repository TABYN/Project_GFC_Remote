...
class Bordereau(models.Model):
    ...
    etat_borderau = models.BooleanField(default=True, null=True, blank=True)  #Rejet� ou accept�
...
class Credit_S2(models.Model):
    ...
    credit_allouee = MoneyField(decimal_places=2, max_digits=9)
    credit_reste = MoneyField(decimal_places=2, max_digits=9)
    epc = models.BooleanField(default=False, null=True, blank=True)  #Engag� prise en compte ou non
...
class Engagement(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.pk:
            # Si l'objet n'a pas encore de cl� primaire, c'est qu'il s'agit d'une cr�ation
           last_object = Engagement.objects.filter(credit_alloue__article=self.credit_alloue.article).order_by('-num').first()
         
           if last_object:
                # Si des objets existent d�j�, r�cup�rez le plus grand nombre et incr�mentez-le de 1
              self.num = last_object.num + 1
           else:
                # Si aucun objet n'existe, commencez � 1
                self.num = 1
...
#methode pour calculer le totale des montatnts des mandats
    ...
    def total_montant(self):
        ...
        print('atika')
        print(self)
        # Vous pouvez obtenir les mandats li�s � cet engagement directement
...
class Mandat(models.Model):
 ...
 ###########################################  
    def total_montant_for_engagement(self):
        # Obtenez l'engagement li� � ce mandat
        engagement = self.credit_s2.article

        # Obtenez tous les mandats li�s � l'engagement et � l'article
