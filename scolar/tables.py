...
class PFETable(tables.Table):
    ...
    action='{% load icons usergroup %}\
            ...
            {% if user.is_authenticated %}\
                ...
                {% elif record.statut_validation == "V" %}\
                    ...
                    {% if user.is_enseignant and not record.groupe %}\
                         {% if request.user.enseignant in record.coencadrants.all %}\
    ...
    class Meta:
        model= PFE
        fields = ('id', 'groupe__section__formation__annee_univ__annee_univ', 'reserve_pour', 'intitule', 'organisme__sigle', 'coencadrants', 'email_promoteur', 'nb_avis', 'statut_validation')
        template_name= "django_tables2/bootstrap4.html"
...
class PFEFilter(django_filters.FilterSet):
    ...
    class Meta:
        model = PFE
        fields = ('annee_universitaire', 'id', 'type', 'keyword', 'pays', 'statut_validation',)
...
class ExamenTable(tables.Table):
    ...
    class Meta:
        model= Seance
        fields = ['activite__type', 'activite__module', 'date', 'heure_debut', 'heure_fin']
        template_name= "django_tables2/bootstrap4.html"
...
class ExamenFilter(django_filters.FilterSet):
    ...
    semestre = django_filters.ModelChoiceFilter(field_name='activite__module__periode__periode', queryset=Periode.objects.all().order_by('code'), label='Semestre', empty_label='Semestre')
    ...
    class Meta:
        model = Seance
        fields = ['annee_univ', 'semestre', 'type', 'matiere']
...
class InscriptionEtudiantTable(tables.Table):
    ...
    action= '{% load icons %}\
                {% if user.is_etudiant %}\
                    ...
                    {% endif %}\
                {% elif user.is_direction or user.is_scolarite or user.is_surveillance %}\
                    <a href="{% url "fiche_inscription_pdf" inscription_pk=record.id %}">Fiche {% icon "file-pdf" %}</a>\
                {% endif %}\
...
class PVTable(tables.Table):
    ...
    class Meta:
        model = PV
        fields = ['date', 'annuel', 'periode', 'note_eliminatoire', 'moy_ue', 'rang', 'tri_rang', 'photo', 'signature', 'anonyme', 'reserve']
        template_name= "django_tables2/bootstrap4.html"
...
#                 <a href="{% url "article_update" pk=record.id %}" > {% icon "pencil-alt" %} </a>'          
#     declarer_posteriori= tables.TemplateColumn(action, orderable=False)
    action= '<a href="{% url "test" crd=record.id %}" class="btn btn-info" role="button"> Mandatement</a>' #% url "test" mandat_pk=record.id %}" 
    Mandatement=tables.TemplateColumn(action, orderable=False)

    class Meta:
        model =Credit_S2
        fields=('chapitre','article__code_art', 'article__libelle_art_FR')#'id',
        template_name= "django_tables2/bootstrap4.html"   
